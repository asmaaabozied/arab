...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            ...
            <form
                method="POST"
                action="{{route('employer.send.ticket')}}"
                    ...
                    <div class="card-body p-3 pt-0">
                        <div class="row">
                            <div class="col-12 my-2">
                                <h6 class="mb-2 ">{{trans('employer::ticket.support section')}}</h6>
                                <select id="section-dropdown"  name="section"
                                        class="form-control "
                                        required>
                                    <option value="">{{trans('employer::ticket.pleas_select_support_section')}}</option>
                                    @if(count($sections) > 0)
                                        @if(app()->getLocale() == "ar")
                                        ...
                                        @endif
                                    @else
                                        <option>{{trans('employer::ticket.NoSectionFound')}}</option>
                                    @endif
                            ...
                            <div class="col-12 my-2 ">
                                ...
                                <input name="subject" value="{{old('subject')}}" required
                                       ...
                                       placeholder="{{trans('employer::ticket.PleasEnterTicketSubject')}}">
                            ...
                            <div class="col-12 my-2">
                                ...
                                <textarea name="description" required
                                          ...
                                          placeholder="{{trans('employer::ticket.PleasEnterTicketDescription')}}">{{old('description')}}</textarea>
                            </div>
                            <div class="col-lg-12 col-md-12 col-12">
                                <h6 class=" text-center">{{trans('employer::ticket.TicketAttachedFiles')}}</h6>
                                <style>
                                    .center {
                                        height: 100%;
                                        display: flex;
                                        ...
                                        <div class="center">
                                            <div class="image-form-input">
                                                ...
                                                <label class="btn btn-dark btn-lg w-100"
                                                       for="file-ip-1">{{trans('employer::ticket.Upload Attached File')}}</label>
                                                ...
                                                <button class="btn btn-danger btn-lg w-100 d-none my-2 " id="remove"
                                                        onclick="removeImage()">{{trans('employer::ticket.Remove Attached File')}}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="button-row ">
                    <button type="submit"
                            class="btn btn-primary btn-lg w-100   mb-2">{{trans('employer::ticket.createTicket')}}</button>
                </div>
    ...
    <script>
        $(document).ready(function () {
            $('#section-dropdown').on('change', function () {
                ...
                $.ajax({
                    url: "{{route('employer.fetch.section.details.when.create.ticket')}}",
                    type: "POST",
                    data: {
