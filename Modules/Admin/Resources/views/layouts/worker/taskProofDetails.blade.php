        ...
        <div class="card">
            <div class="row">
                <div class="card-body ar-text-right ">
                    ...
                    @if($proof->task->proof_request_screenShot !=Null)
                        <h6 class="mb-0 font-weight-bolder"> {{$proof->task->proof_request_screenShot}} </h6>
                    @else
            ...
            <div class="col-md-12 ">
                <div class="card">
                    <div class="card-body ar-text-right ">
                        ...
                        @if($proof->task->proof_request_ques !=Null)
                            <h6 class="mb-0 font-weight-bolder"> {{$proof->task->proof_request_ques}} </h6>
                        @else
...
<div class="row mt-4">
    <div class="button-row ">
        <a href="{{route('admin.show.worker.profile',$proof->worker->id)}}"
...
@if($proof->isAdminAcceptProof == "NotSeenYet")
    <div class="row mt-4">
        ...
        <div class="button-row ">
            <a href="{{route('admin.worker.approve.task.proof',[$proof->id,$proof->task_id])}}"
               class="btn bg-gradient-success btn-lg w-100 mb-0">{{trans('employer::task.EmployerAcceptedProf')}}</a>
        </div>
    </div>
    ...
    <div class="row mt-4">
        <div class="button-row ">
            <button id="rejectBtn"
                    class="btn bg-gradient-danger btn-lg w-100 mb-0">{{trans('employer::task.EmployerRejectedProf')}}</button>
        </div>
        <form class="d-none"  method="POST" id="rejectForm"  action="{{route('admin.worker.reject.task.proof',[$proof->id,$proof->task_id])}}"
