        ...
        <div class="col-lg-8 mx-auto">
            <div class="card mb-4 bg-white border-0">
                <div class="card-header p-3 pb-0 bg-white border-0">
                    <div class="d-flex justify-content-between align-items-center ">
                        <div>
                            <h6>{{trans('worker::ticket.TicketDetails')}}</h6>
                            <p class="text-lg my-1">
                                {{trans('worker::ticket.Ticket no.')}} <b>{{$data->ticket_number}}</b>
                            </p>
                            <p class="text-lg my-1">
                                {{trans('worker::ticket.TicketCreatedAt')}} <b>{{$data->created_at}}</b>
                            </p>
                            <p class="text-lg my-1">
                                @if(app()->getLocale() == "ar")
                                    {{trans('worker::ticket.TicketSupportSection')}}
                ...
                <div class="card-body p-3 pt-0">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-md-6 col-12 mt-4">
                            <h6 class="mb-3 text-center text-md fw-bold">{{trans('worker::ticket.TicketStatuses')}}</h6>
                                ...
                                @foreach($data->statuses as $status)
                                    <div class="timeline-block mb-3">
                                        ...
                                        <div class="timeline-content">
                                            <h6 class="text-dark text-sm font-weight-bold mb-0">{{trans('worker::ticket.'.$status->name->name)}}</h6>
                        ...
                        <div class="col-lg-8 col-md-6 col-12 mt-4">
                            <h6 class="mb-3 text-center text-md fw-bold">{{trans('worker::ticket.TicketAttachedFiles')}}</h6>
                ...
                <div class="card-body p-3 pt-1">
                    <h6 class="mb-3 text-center text-sm">{{trans('worker::ticket.TicketAnswers')}}</h6>
                        ...
                        <div class="card-body overflow-auto overflow-x-hidden">
                            ...
                            @else
                                <div id="no-message-found"
                                     class="col-12 text-warning text-center font-elmessiry"> {{trans('worker::ticket.No Messages Found')}}</div>
                            @endif
                ...
                @if($data->statuses()->with('name')->latest()->first()->name->name == "AdminAnswered")
                    <div class="card-footer d-block">
                        <form
                            method="POST"
                            action="{{route('worker.send.answer',$data->id)}}"
                            ...
                            <div class="d-flex">
                                <div class="input-group">
                                    <input id="ans-input-box" type="text" class="form-control"
                                           placeholder="{{trans('worker::ticket.Type Answer Here')}}"
                                           aria-label="Employer Answer" name="worker_answer" onfocus="focused(this)"
            ...
            <div class="button-row ">
                <a href="{{route('worker.show.my.tickets')}}"
                   class="btn btn-primary btn-lg w-100 text-white  mb-2">{{trans('worker::task.back')}}</a>
            </div>
        </div>
    </div>
